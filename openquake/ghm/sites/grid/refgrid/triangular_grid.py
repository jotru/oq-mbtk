...
def get_midp(lo1, la1, lo2, la2):
    ...
    :param la1:
    """
    x1, y1, z1 = ll2xyz(lo1, la1)
    x2, y2, z2 = ll2xyz(lo2, la2)
    x = np.mean([x1, x2])
    y = np.mean([y1, y2])
    z = np.mean([z1, z2])
    ...
    y = y/mag
    z = z/mag
    loc, lac = xyz2ll(x, y, z)
    return loc, lac


def ll2xyz(lon, lat):
    h = np.cos(np.deg2rad(lat))
    z = np.sin(np.deg2rad(lat))
    y = np.sin(np.deg2rad(lon))*h
    x = np.cos(np.deg2rad(lon))*h
    mag = np.sqrt(x*x + y*y + z*z)
    x = x/mag
    y = y/mag
    z = z/mag
...
def xyz2ll(x, y, z):
    ...
    x = x/mag
    y = y/mag
    lon = np.rad2deg(np.arctan2(y, x)) if abs(x) > 1e-20 else 90 * np.sign(x*y)
    lat = np.rad2deg(np.arcsin(z))
    return lon, lat


def dst(lo1, la1, lo2, la2):
    """
    :param lo1:
    :param lo2:
    """
...
class RegularTriangularGrid(ReferenceGrid):
    ...
    def split_facets(self):
        ...
        for i in facet_indexes:
            ...
            loc0, lac0 = get_midp(pnt0[0], pnt0[1], pnt1[0], pnt1[1])
            loc1, lac1 = get_midp(pnt1[0], pnt1[1], pnt2[0], pnt2[1])
            loc2, lac2 = get_midp(pnt0[0], pnt0[1], pnt2[0], pnt2[1])

            if 1:
                px0, py0, pz0 = ll2xyz(pnt0[0], pnt0[1])
                px1, py1, pz1 = ll2xyz(pnt1[0], pnt1[1])
                px2, py2, pz2 = ll2xyz(pnt2[0], pnt2[1])
                x0, y0, z0 = ll2xyz(loc0, lac0)
                x1, y1, z1 = ll2xyz(loc1, lac1)
                x2, y2, z2 = ll2xyz(loc2, lac2)
                #
                # check that the sides of the original triangle have similar length
                s0 = dst(pnt0[0], pnt0[1], pnt1[0], pnt1[1])
                s1 = dst(pnt2[0], pnt2[1], pnt1[0], pnt1[1])
                #print(abs(s0-s1), 0.02*s0, s0, s1, 'ratio', abs(s0-s1)/s0)
                assert abs(s0-s1) < 0.2*s0
                s2 = dst(pnt2[0], pnt2[1], pnt0[0], pnt0[1])
                assert abs(s0-s2) < 0.2*s0
                #
                # check mid points
                d0a = dst(pnt0[0], pnt0[1], loc0, lac0)
                d1a = dst(pnt1[0], pnt1[1], loc0, lac0)
                assert abs(d0a-d1a) < 0.1*d0a
                d0b = dst(pnt1[0], pnt1[1], loc1, lac1)
                d1b = dst(pnt2[0], pnt2[1], loc1, lac1)
                assert abs(d0b-d1b) < 0.1*d0b
                d0c = dst(pnt0[0], pnt0[1], loc2, lac2)
                d1c = dst(pnt2[0], pnt2[1], loc2, lac2)
                assert abs(d0c-d1c) < 0.1*d0c
                #
                # check new triangle
                d0t = dst(loc0, lac0, loc1, lac1)
                d1t = dst(loc1, lac1, loc2, lac2)
                assert abs(d0t-d1t) < 0.2*d0t
                d2t = dst(loc0, lac0, loc2, lac2)
                # print(abs(d0t-d2t), d0t, d2t, 'ratio', abs(d0t-d2t)/d0t)
                assert abs(d0t-d2t) < 0.2*d0t
            ...
            if OPTINS == 1:
                ...
                idx0 = self.coords.insvtx([loc0, lac0])
                idx1 = self.coords.insvtx([loc1, lac1])
                idx2 = self.coords.insvtx([loc2, lac2])
            #
            ...
            if first:
                distance = dst(pnt0[0], pnt0[1], loc0, lac0) * RADIUS
                first = False
